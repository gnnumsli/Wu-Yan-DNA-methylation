...
def load_dataset(data_path):
    ...
    sequence_list, label_list = load_tsv_format_data(data_path)
    ...
    for i in range(len(sequence_list)):
        ...
        if len(sequence_cur) < 41:
            print('The input sequence ''%s'' does not meet the minimum length requirement of 40.' % sequence_list[i])
        else:
    ...
    One_hot_matrix_input = torch.from_numpy(One_hot_matrix_input)

    NCP_matrix_input = np.asarray([i for i in NCP_matrix_list], dtype=np.float32)
    NCP_matrix_input = torch.from_numpy(NCP_matrix_input)

    DPCP_matrix_input = np.asarray([i for i in DPCP_matrix_list], dtype=np.float32)
    DPCP_matrix_input = torch.from_numpy(DPCP_matrix_input)

    label_list_input = np.asarray([i for i in label_list], dtype=np.int64)
    label_list_input = torch.from_numpy(label_list_input)

    all_matrix_iinput = torch.cat((One_hot_matrix_input, NCP_matrix_input, DPCP_matrix_input), dim=1)



    return sequence_list, label_list_input, One_hot_matrix_input, NCP_matrix_input, DPCP_matrix_input, all_matrix_iinput

# path= ".\Data_DNA\\6mA\\6mA_A.thaliana\\train.tsv"
